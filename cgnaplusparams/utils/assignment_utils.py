...
def params_to_names(params: np.ndarray) -> list[str]:
    """
    Generate DOF names from the shape of a parameters array.

    Infers the sequence length from the number of degrees of freedom in the
    input array and returns the corresponding DOF names via cgnaplus_name_assignment.
    The sequence length is determined by solving 4*N - 3 = n_dofs, where n_dofs
    is the first dimension of the input array.

    Parameters
    ----------
    params : np.ndarray
        Parameters array with shape (n_dofs, 6) or higher dimensions where the
        first dimension is n_dofs. Must satisfy n_dofs = 4*N - 3 for some
        positive integer N (the sequence length).

    Returns
    -------
    list[str]
        List of DOF names generated via cgnaplus_name_assignment for a sequence
        of length N.
    """
    return cgnaplus_name_assignment("A" * params_to_nbp(params))

def params_to_nbp(params: np.ndarray) -> int:
    """
    Infers the sequence length (number of base pairs) from the shape of a parameters array.

    Determines the number of base pairs N from the first dimension n_dofs of the input
    array by solving the equation n_dofs = 4*N - 3, which arises from the structure of
    the DOF list generated by cgnaplus_name_assignment.
    ...
    -------
    int
        The inferred sequence length N.
    """
    if params.ndim == 1:
        raise ValueError(f"Input params array must have at least 2 dimensions, with shape (n_dofs, 6), but got shape {params.shape}.")
    if (params.shape[-2] + 3) % 4 != 0:
        raise ValueError("Invalid number of parameters. Needs to be of the form 4*N - 3 for some integer N.")
    return (params.shape[-2] + 3) // 4
...
def nonphosphate_dof_map(seq: str, param_names: list[str] | None = None) -> np.ndarray:
    ...
    A DOF is considered non-phosphate if its name contains INTRA_BP_PARAM_NAME or INTER_BP_PARAM_NAME,
...
def dof_index_from_name(
    ...
    Args:
        dof_name: DOF name string, e.g. ``"b3"`` or ``"W1"``.
...
def inter_bp_dof_indices(param_names: list[str]) -> np.ndarray:
    ...
    in INTER_BP_PARAM_NAME in their names.
...
def intra_bp_dof_indices(param_names: list[str]) -> np.ndarray:
    ...
    in INTRA_BP_PARAM_NAME in their names.
...
def watson_phosphate_dof_indices(param_names: list[str]) -> np.ndarray:
    ...
    in B2P_WATSON_PARAM_NAME in their names.
...
def crick_phosphate_dof_indices(param_names: list[str]) -> np.ndarray:
    ...
    in B2P_CRICK_PARAM_NAME in their names.
...
def phosphate_dof_indices(param_names: list[str]) -> np.ndarray:
    ...
    substrings defined in B2P_WATSON_PARAM_NAME
    or B2P_CRICK_PARAM_NAME in their names.
